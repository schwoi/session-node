...
const state = { token: sessionStorage.getItem('manager-token') || '', timer: null, busy: false, generation: 0, reading: false };
...
function overall(node) {
  ...
  if (node.sample && !node.sample.at) return ['warn', 'awaiting sample'];
  if (node.sample && Date.now() - Date.parse(node.sample.at) > ((node.sample.interval_seconds || 300) + 60) * 1000) {
    return ['warn', 'stale sample'];
  }
...
function facts(node) {
  ...
  const list = [
    ['Last sampled', node.sample?.at ? `${duration((Date.now() - Date.parse(node.sample.at)) / 1000)} ago` : 'Waiting for scheduled sample'],
...
function services(node) {
  ...
  const rows = node.processes.map((process) => {
    ...
    if (process.name in pings) {
      detail = age == null ? 'never reported' : `${duration(age)} before sample`;
...
function render(payload) {
  ...
  $('#refreshed').textContent = `Showing saved samples · poll every ${duration(payload.interval_seconds || 300)}`;
  if (payload.collection_error) showMessage(payload.collection_error);
  ...
  for (const group of groups) {
    ...
    if (!group.nodes.length && !group.error) {
      grid.append(element('p', 'muted', 'Waiting for node discovery. New containers are discovered every 30 seconds.'));
    }
...
function renderCard(grid, node) {
  const template = $('#node-card');
  {
    ...
    if (node.error) {
      ...
      $('.actions', card).append(button('Update now', () => forceUpdate(node)));
      grid.append(card);
      return;
    }
    $('.tags', card).append(element('span', 'tag', node.network), element('span', 'tag', node.role));
    if (node.problems.length && node.sample?.state !== 'pending') {
      problems.hidden = false;
...
function actions(node) {
  ...
  const update = button(node.sample?.refreshing ? 'Update queued' : 'Update now', () => forceUpdate(node));
  update.disabled = Boolean(node.sample?.refreshing);
  const list = [update];
  if (running) {
...
async function forceUpdate(node) {
  try {
    const result = await api(`${nodePath(node)}/refresh`, { method: 'POST', body: {} });
    await refresh();
    showMessage(result.refresh === 'cooldown'
      ? `${node.name} was sampled recently. Please allow ${result.cooldown_seconds} seconds between updates.`
      : `${node.name}: update queued. The saved sample will change when collection finishes.`);
  } catch (error) {
...
function schedule() {
  ...
  state.timer = null;
  if ($('#auto').checked) state.timer = setInterval(() => {
